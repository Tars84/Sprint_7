...
public class NotCreateCourierTest {
    ...
    private final Courier courierWithLoginOnly = CourierCreate.getCourierWithLoginOnly();
    private final Courier courierWithPasswordOnly = CourierCreate.getCourierWithPasswordOnly();
    ...
    public void notLoginCourierWithOutPassword() {
        ...
        ValidatableResponse responseLogin = courierClient.login(InputData.from(courier));
        id = responseLogin.extract().path("id");
        ValidatableResponse responseLoginIncorrect = courierClient.login(InputDataOnlyLogin.fromOnlyLogin(courier));
        assertTrue(responseCreate.extract().statusCode() == 201
    ...
    public void notLoginCourierWithOutLogin() {
        ...
        ValidatableResponse responseLogin = courierClient.login(InputData.from(courier));
        id = responseLogin.extract().path("id");
        ValidatableResponse responseLoginIncorrect = courierClient.login(InputDataOnlyPassword.fromOnlyPassword(courier));
        assertTrue(responseCreate.extract().statusCode() == 201
                && responseLoginIncorrect.extract().statusCode() == 400
                && responseLoginIncorrect.extract().path("message").equals("Недостаточно данных для входа"));
    }
